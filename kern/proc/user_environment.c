...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    pt_set_page_permissions(ptr_user_page_directory, (uint32)ptr_stack, 0, PERM_PRESENT);
    if(ptr_stack == NULL){
    	panic("invalid ptr");
    }
    return (void*)ptr_stack;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->uheap_start = (uint32*)daStart;
	e->uheap_limit = (uint32*)daLimit;
	e->uheap_break = (uint32*)daStart;
