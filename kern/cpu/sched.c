...
struct Env* fos_scheduler_PRIRR()
{
	...
	//Comment the following line
	//panic("Not implemented yet");
	struct Env* currenv = get_cpu_proc();
		...
		for(uint8 i=0 ;i<num_of_ready_queues;i++){
			...
			if(size>0){
				found = dequeue(&(ProcessQueues.env_ready_queues[i]));
				if(found !=NULL){
				found->ticko=0;
				}
				break ;
	 ...
	 if (isSchedMethodPRIRR())
		    {
		        ...
		        if (timer_ticks() > starv_Thresh){
		        	for (int i = 1 ; i < num_of_ready_queues; i++)
					{
						struct Env_Queue* queue = &ProcessQueues.env_ready_queues[i];
						struct Env* envo = NULL;
						LIST_FOREACH(envo, queue)
						{
							//envo->ticko+=1;
	//		                cprintf("ticks: %d \n", envo->ticko);
								sched_remove_ready(envo);
//								if (envo->priority > 0)
								envo->priority -= 1;
								sched_insert_ready(envo);
