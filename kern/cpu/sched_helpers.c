...
{
    // Validate priority (ensure it's non-negative)
    if (priority < 0) {
        cprintf("Invalid priority %d for environment %d\n", priority, envID);
        return;
    }

    // Get the environment with the given ID
    struct Env* proc;
    int result = envid2env(envID, &proc, 0);
    if (result != 0 || proc == NULL) {
        cprintf("Invalid environment ID %d\n", envID);
        return;
    }
    ...
    for (uint8 i = 0; i < num_of_ready_queues; i++) {
        current_env = find_env_in_queue(&ProcessQueues.env_ready_queues[i], envID);
        if (current_env != NULL) {
