...
uint32 sys_check_permissions(uint32 virtual_address)
{
	return check_user_mem(cur_env,virtual_address);
}
...
uint32 sys_signal(uint32 queue){
//	//cprintf("signal         /////////////////////////////////\n");
//	acquire_spinlock(&ProcessQueues.qlock);
//	 //cprintf("entered signal 2");
//	struct Env* env_to_unblock = dequeue((struct Env_Queue*)queue);
//	if (env_to_unblock != NULL) {
//		//env_to_unblock->env_status = ENV_READY;
//		//sched_insert_ready(env_to_unblock);
//		assert(env_to_unblock != NULL);
//		{
//			//cprintf("\nInserting %d into ready queue 0\n", env->env_id);
//			env_to_unblock->env_status = ENV_READY ;
//			enqueue(&(ProcessQueues.env_ready_queues[env_to_unblock->priority]), env_to_unblock);
//		}
//	}
//	//semdata->lock=0;
//	release_spinlock(&ProcessQueues.qlock);
...
//--------------------------------------------------
void sys_wait_semaphore(uint32 queue, uint32 semdata){
    //cprintf("first---------------------------------------\n");
    acquire_spinlock(&ProcessQueues.qlock);
    //struct Env* current_env = get_cpu_proc();
    //current_env->env_status = ENV_BLOCKED;
...
uint32 sys_signal_semaphore(uint32 queue){
	//cprintf("signal         /////////////////////////////////\n");
	acquire_spinlock(&ProcessQueues.qlock);
	 //cprintf("entered signal 2");
	struct Env* env_to_unblock = dequeue((struct Env_Queue*)queue);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_wait_semaphore:
		sys_wait_semaphore(a1,a2);
		return 0;
		break;
	case SYS_signal_semaphore:
		return sys_signal_semaphore(a1);
		break;
	case SYS_wait:
		sys_wait((uint32)a1,(uint32) a2);
		return 0;
		break;
	case SYS_signal:
		return sys_signal((uint32)a1);
		break;
	case SYS_env_set_priority:
		env_set_priority(a1,a2);
		return 0;
		break;
	case SYS_check_permissions:
		return sys_check_permissions((uint32)a1);
		break;
	case SYS_sbrk:
		return (int) sys_sbrk((uint32) a1);
		break;
