...
int is_subsequence(const char* str1, const char* str2) {
    while (*str2) {
        ...
        if (*str1 == '\0') {
            return 1; // All characters of str1 matched in order
        }
        str2++;
    }
    return 0; // str1 is not a subsequence of str2
}
	 ...
	 for (int i = 0; i < NUM_OF_COMMANDS; i++)
	 {
	  if (strcmp(arguments[0], commands[i].name) == 0)
	  {
	   if((number_of_arguments - 1 == commands[i].num_of_args) ||
	     (number_of_arguments > 0 && commands[i].num_of_args == -1)){
	    return i;
	   }
	   else{
	  ...
	  else if(is_subsequence(arguments[0], commands[i].name)){
