...
void _main(void)
{
	...
	cprintf("\n%~[5] Allocate again [test first fit in coalesced area] [15%]\n");
	{
		...
		if ((uint32) ptr_allocations[13] != (pagealloc_start + 1*Mega + 768*kilo)) { is_correct = 0;
		cprintf("returned is: %u, expected : %u\n",(uint32) ptr_allocations[13],(pagealloc_start + 1*Mega + 768*kilo) );
